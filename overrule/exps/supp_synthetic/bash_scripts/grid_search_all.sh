...
for rep in {1..2}
do
  for outer_args in "nr=10 nn=10 K=10 COVERAGE=False B="{10}" LAMBDA1="{1e-2}
  do
    wait
    for inner_args in "LAMBDA0="{1e-2}" ALPHA="{0.95}
      do
        echo `date +"%Y-%m-%d %T"` "with SEED=${rep} ${outer_args} ${inner_args}"
        THIS_DIR=${RESULT_DIR}_${inner_args// /_}
        /usr/bin/python3 run_experiment.py -t ${THIS_DIR} -l INFO with SEED=$rep $outer_args $inner_args &
      done
  done
done
